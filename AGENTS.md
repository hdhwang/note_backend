...
- **Suspicious Activity Monitoring Skill (의심 활동 모니터링 스킬)**
  - **기능**: `AuditLog` 데이터를 실시간/배치로 분석하여 비정상적인 패턴(예: 짧은 시간 내 다수의 로그인 실패, 평소와 다른 IP 대역에서의 대량 데이터 조회 및 삭제)을 탐지.
...
- **Account Usage Report Skill (계좌 활용 리포트 스킬)**
  - **기능**: 등록된 은행 계좌와 설명(`description`) 데이터를 바탕으로 사용자의 자산 풀을 정리하고, 대시보드 API(`DashboardStatsAPI`)와 연계하여 요약 브리핑 생성.

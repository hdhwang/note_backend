...
- **Vector Database Integration Skill (벡터 DB 검색 스킬)**
  - **기능**: 노트 내용, 시리얼 제품 설명 등을 임베딩(Embedding)하여 Vector DB(예: Pinecone, Milvus, pgvector)에 저장. 통합된 `created_at` 필드를 메타데이터로 함께 인덱싱하여 시간/날짜 기반 필터링을 지원.
  - **활용**: 사용자가 "작년에 샀던 영상 편집 프로그램 키가 뭐지?" 혹은 "최근 1주일 내에 등록한 계좌번호 알려줘"라고 자연어로 검색하면 의미와 시계열 메타데이터를 결합 분석하여 정확한 데이터를 반환.
...
## 6. 📂 Data Export & Analysis Support (데이터 추출 및 분석 지원)

최근 구현된 데이터 내보내기(Excel/ZIP) 기능을 활용하여 AI 에이전트의 분석 범위를 확장합니다.

- **Batch Analysis Skill (배치 분석 스킬)**
  - **기능**: 특정 기간의 `AuditLog`나 `GuestBook` 데이터를 엑셀 형태로 추출하여 LLM에게 대량의 컨텍스트로 제공하거나, 시계열 분석을 수행하여 인사이트 리포트 생성.
  - **장점**: API 호출 횟수를 줄이고 대규모 데이터를 구조화된 형태로 일괄 처리 가능.
